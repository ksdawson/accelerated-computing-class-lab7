...
struct __align__(8) RleData {
    uint32_t count;         // number of runs in segment
    char first;             // first value in segment
    char last;              // last value in segment
};
static_assert(sizeof(RleData) == 8, "RleData must be 8 bytes");

struct RleOp {
    using Data = RleData;

    static __host__ __device__ __forceinline__ Data identity() { return {0, '\0', '\0'}; }
    ...
    static __host__ __device__ __forceinline__ Data combine(Data a, Data b) {
        ...
        out.first = a.first;
        out.last = b.last;
        out.count = a.count + b.count - (a.last == b.first ? 1 : 0);
        return out;
    }

    static __host__ __device__ __forceinline__ Data combine(Data a, char b) {
        Data out;
        out.first = a.first;
        out.last = b;
        out.count = (a.last == b) ? a.count : a.count + 1;
        return out;
    }
};
...
__device__ inline typename Op::Data thread_local_scan(size_t n, char const *x, typename Op::Data *out,
    const uint32_t start_i, const uint32_t end_i,
    typename Op::Data accumulator
) {
    using Data = typename Op::Data;
    using VecData = Vectorized<Data, VEC_SIZE>;
    using VecChar = Vectorized<char, VEC_SIZE>;

    // Vectorize
    VecChar const *vx = reinterpret_cast<VecChar const *>(x);
    ...
    for (uint32_t i = start_vi; i < end_vi; ++i) {
        ...
        for (uint32_t vi = 0; vi < VEC_SIZE; ++vi) {
            accumulator = Op::combine(accumulator, vchar.elements[vi]);
            vdata.elements[vi] = accumulator;
        }
        // Output to memory
        if constexpr (DO_FIX) { vout[i] = vdata; }
    }
    // Handle vector tail
...
__device__ typename Op::Data warp_scan(
    size_t n, char const *x, typename Op::Data *out, // Work dimensions
...
__device__ void warp_scan_handler(
    ...
    for (uint32_t idx = 0; idx < num_blocks; ++idx) {
        // Move buffers
        char const *bx = x + idx * block_size;
...
template <typename Op, uint32_t VEC_SIZE, bool DO_FIX>
__global__ void local_scan(size_t n, char const *x, typename Op::Data *out, typename Op::Data *seed) {
    using Data = typename Op::Data;
    // Thread block info
    ...
    char const *sm_x = x + block_idx * n_per_sm;
    ...
    char const *warp_x = sm_x + warp_idx * n_per_warp;
...
template <typename Op> size_t get_workspace_size(size_t n) {
    using Data = typename Op::Data;
    return 48 * 8 * sizeof(Data); // num sms x num warps
}

template <typename Op>
typename Op::Data *launch_scan(
    size_t n,
    char const *x, // pointer to GPU memory
    typename Op::Data *out, // pointer to GPU memory
...
__global__ void extract_compressed_data(
    size_t rcn, typename Op::Data *run_counts,
    size_t cdn, typename Op::Data *out
) {
    // Data types
    using Data = typename RleOp::Data;
    using VecData = Vectorized<Data, VEC_SIZE>;
    VecData const *vrun_counts = reinterpret_cast<VecData*>(run_counts);

    // Handle first element
    if (blockIdx.x == 0 && threadIdx.x == 0) {
        const VecData vcurr = vrun_counts[0];
        const Data curr = vcurr.elements[0];
        out[curr.count - 1] = {0, '\0', curr.last};
        // Handle rest
        #pragma unroll
        for (uint32_t vi = 1; vi < VEC_SIZE; ++vi) {
            Data prev = vcurr.elements[vi - 1];
            Data curr = vcurr.elements[vi];
            if (prev.last != curr.last) {
                // Start of run
                out[curr.count - 1] = {vi, '\0', curr.last};
            }
        }
    }

    // Iterate over all run counts and check if its the start of a run
    for (uint32_t idx = blockIdx.x * blockDim.x + threadIdx.x + 1; idx < rcn / VEC_SIZE; idx += gridDim.x * blockDim.x) {
        const VecData vprev = vrun_counts[idx - 1];
        const VecData vcurr = vrun_counts[idx];
        // Handle first element
        const Data prev = vprev.elements[VEC_SIZE - 1];
        const Data curr = vcurr.elements[0];
        if (prev.last != curr.last) {
            // Start of run
            out[curr.count - 1] = {idx * VEC_SIZE, '\0', curr.last};
        }
        // Handle rest
        #pragma unroll
        for (uint32_t vi = 1; vi < VEC_SIZE; ++vi) {
            ...
            if (prev.last != curr.last) {
                // Start of run
                out[curr.count - 1] = {idx * VEC_SIZE + vi, '\0', curr.last};
            }
        }
    }
    ...
    // Handle tail
    if (blockIdx.x == 0 && threadIdx.x == 0) {
        const uint32_t start_scalar_i = (rcn / VEC_SIZE) * VEC_SIZE;
        for (uint32_t i = start_scalar_i; i < rcn; ++i) {
            Data prev = run_counts[i - 1];
            Data curr = run_counts[i];
            if (prev.last != curr.last) {
                // Start of run
                out[curr.count - 1] = {i, '\0', curr.last};
            }
        }
    }
}
...
template <typename Op, uint32_t VEC_SIZE>
__global__ void extract_compressed_lengths(uint32_t raw_count,
    size_t cdn, typename Op::Data *cd,
    ...
    VecData const *vcd = reinterpret_cast<VecData*>(cd);
    for (uint32_t idx = blockIdx.x * blockDim.x + threadIdx.x; idx < cdn / VEC_SIZE - 1; idx += gridDim.x * blockDim.x) {
        const VecData vcurr = vcd[idx];
        const VecData vnext = vcd[idx + 1];
        #pragma unroll
        for (uint32_t vi = 0; vi < VEC_SIZE - 1; ++vi) {
            const Data curr = vcurr.elements[vi];
            const Data next = vcurr.elements[vi + 1];
            compressed_data[idx * VEC_SIZE + vi] = curr.last;
            compressed_lengths[idx * VEC_SIZE + vi] = next.count - curr.count;
        }
        // Handle last element
        const Data curr = vcurr.elements[VEC_SIZE - 1];
        const Data next = vnext.elements[0];
        compressed_data[idx * VEC_SIZE + VEC_SIZE - 1] = curr.last;
        compressed_lengths[idx * VEC_SIZE + VEC_SIZE - 1] = next.count - curr.count;
    }

    // Handle last element
    if (blockIdx.x == 0 && threadIdx.x == 0) {
        // Handle tail
        const uint32_t start_scalar_i = (cdn / VEC_SIZE - 1) * VEC_SIZE;
        for (uint32_t i = start_scalar_i; i < cdn - 1; ++i) {
            const Data curr = cd[i];
            const Data next = cd[i + 1];
            compressed_data[i] = curr.last;
            compressed_lengths[i] = next.count - curr.count;
        }
        // Handle last element
        const Data curr = cd[cdn - 1];
        compressed_data[cdn - 1] = curr.last;
        compressed_lengths[cdn - 1] = raw_count - curr.count;
    }
}
...
size_t get_workspace_size(uint32_t raw_count) {
    ...
    const size_t run_counts_size = (16 << 20) * sizeof(Data);
    const size_t compressed_data_size = (16 << 20) * sizeof(Data);
    return scan_size + run_counts_size + compressed_data_size;
}
...
uint32_t launch_rle_compress(
    ...
    void *seed = workspace;
    Data *rcnt_out = reinterpret_cast<Data*>(seed) + 48 * 8;
    Data *cd_out = rcnt_out + (16 << 20);

    // Run scan of raw
    scan_gpu::launch_scan<RleOp>(raw_count, raw, rcnt_out, seed);

    // Launch a kernel to extract the compressed data
    extract_compressed_data<RleOp, 4><<<48, 8*32>>>(raw_count, rcnt_out, raw_count, cd_out);

    // Extract the compressed count
    Data last_run;
    CUDA_CHECK(cudaMemcpy(&last_run, &rcnt_out[raw_count - 1], sizeof(Data), cudaMemcpyDeviceToHost));
    uint32_t compressed_count = last_run.count;

    // Launch a kernel to extract the compressed lengths
    extract_compressed_lengths<RleOp, 4><<<48, 8*32>>>(raw_count, compressed_count, cd_out, compressed_data, compressed_lengths);
